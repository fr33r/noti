...
public final class SMSQueueService implements MessageQueueService {
	...
	private class ProducerCallback implements Callback {
		...
		public ProducerCallback(final MetricRegistry metricRegistry, final Logger logger) {
			...
			this.enqueueSuccessMeter = this.metricRegistry.meter(
				MetricRegistry.name(
					SMSQueueService.class,
					SMS_ENQUEUE_SUCCESS_METER_NAME
				)
			);
			this.enqueueFailureMeter = this.metricRegistry.meter(
				MetricRegistry.name(
					SMSQueueService.class,
					SMS_ENQUEUE_FAILURE_METER_NAME
				)
			);
			this.logger = logger;
		}
		...
		TimeoutException
	{
		...
		if(message == null) {
			throw new IllegalArgumentException("Invalid value for 'messageID'. The message does not exist.");
		}
